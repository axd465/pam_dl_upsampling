...
def UNet(input, filters = 32, kernel_size = 3, padding = 'same',
         activation = 'relu', kernel_initializer = 'glorot_normal', strided_conv = False):
...
def DownBlock(input, filters, kernel_size, padding, activation, kernel_initializer, strided_conv):
    ...
    shortcut = out
    if strided_conv:
        out = Conv2D_BatchNorm(out, filters, kernel_size=1, strides=1, padding=padding,
                               activation=activation, kernel_initializer = kernel_initializer)
...
def BridgeBlock(input, filters, kernel_size, padding, activation, kernel_initializer):
    ...
    out = Conv2D_BatchNorm(out, filters//2, kernel_size, strides = 1, padding=padding,
                           activation=activation, kernel_initializer = kernel_initializer)
    out = Conv2D_BatchNorm(out, filters//2, kernel_size=1, strides = 1, padding=padding,
                           activation=activation, kernel_initializer = kernel_initializer)
...
def getModel(input_shape, filters, kernel_size, padding='same', activation='relu',
             kernel_initializer='glorot_normal', strided_conv = True):
